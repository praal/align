...
from environment.kitchen import Kitchen, KitchenState, OBJECTS2, update_facts
...
def evaluate_second_agent(env, policy1, reward1,env2, policy2, reward2):
    ...
    facts = set()
    if env.state.facts[OBJECTS2["very_dirty"]]:
        facts.add(4)
    if env.state.facts[OBJECTS2["dirty"]]:
        facts.add(3)
...
def train(filename, seed):
    ...
    rng2 = Random(seed + 2)
    env2 = Kitchen( map_fn, rng2, 1, 1, problem_mood = problem_mood, allergic=False)
    init2 = create_init([[1,1]])

    rng3 = Random(seed + 3)
    env3 = Kitchen(map_fn, rng3, 1, 1, problem_mood = problem_mood, allergic=True)
            ...
            try:
             ...
             #   print(policy2.Q)
                ...
                policy4 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.2,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng4, others_q=[policy3.get_Q()], others_init=[[1,1]], others_dist=[1.0], penalty=-2*EPISODE_LENGTH, others_alpha=[1.0], objects=OBJECTS2, problem_mood=problem_mood)
                ...
                base_step = evaluate_agent(env3, policy3, reward3, init3)
