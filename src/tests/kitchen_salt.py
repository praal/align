...
from environment.kitchen import Kitchen, KitchenState, OBJECTS4, update_facts
...
def print_state(state, action):
    if action == 0:
        ...
        print("Agent Location:", state.x, state.y,  "Action: Put Up")

    elif action == 5:
        print("Agent Location:", state.x, state.y,  "Action: Put Down")
    else:
    ...
    with open(filename, "w") as csvfile:
        ...
        for j, goal in enumerate(tasks):
            ...
            policy3 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.2,
                                   default_q=DEFAULT_Q, num_actions=5, rng=rng3, others_q=[policy2.get_Q()], others_init=[[1,1]], others_dist=[1.0], penalty=-2*EPISODE_LENGTH, others_alpha=[0.0], objects=OBJECTS4, problem_mood=problem_mood)
            ...
            try:
                ...
                policy1 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.2,
                                       default_q=DEFAULT_Q, num_actions=6, rng=rng1, others_q=[policy2.get_Q()], others_init=[[1,1]], others_dist=[1.0], penalty=-2*EPISODE_LENGTH, others_alpha=[0.1], objects=OBJECTS4, problem_mood=problem_mood)
                ...
                policy4 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.2,
                                       default_q=DEFAULT_Q, num_actions=6, rng=rng4, others_q=[policy3.get_Q()], others_init=[[1,1]], others_dist=[1.0], penalty=-2*EPISODE_LENGTH, others_alpha=[1.0], objects=OBJECTS4, problem_mood=problem_mood)
